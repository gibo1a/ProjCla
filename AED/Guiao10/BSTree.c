...
void* BSTreeGetMin(const BSTree* header) {
  ...
  // ...
  _BSTreeNode *current_node = header->root->left;
  for(int i =0;i<=header->root->left->height;i++){
...
static void* _treeGetMax(const struct _BSTreeNode* root) {
  ...
  // ...
  _BSTreeNode current_node = root->right;
  if(current_node->right==NULL){
    return current_node->item;
  }else{
    _treeGetMax(current_node->right);
  }
...
void* BSTreeSearch(const BSTree* header, const void* item) {
  ...
  while (current != NULL) {
    // COMPLETE THE LOOP.
    // ...
    if(header->compare(item == current->item)){
      return current->item;
    }else{
      if(header->compare(item < current->item)){
