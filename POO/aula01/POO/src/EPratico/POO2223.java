...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) {
		...
		*/
		...
		List<CampingSpace> available = campingService.findAvailableCampingSpaces(SpaceType.CARSPACE, LocalDate.of(2023, 6, 1), 60, new int[]{300, 600});
		...
		available = campingService.findAvailableCampingSpaces(SpaceType.CARSPACE, LocalDate.of(2023, 6, 1),60, new int[]{300, 400});
		if (available==null || available.isEmpty()) {
			out.println("** No options available for the type, dimensions, and dates requested.\n");
		} else {
			CampingSpace u = available.get(0);
